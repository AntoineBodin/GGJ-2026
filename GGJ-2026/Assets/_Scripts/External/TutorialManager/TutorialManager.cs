...
public class TutorialManager : SingletonBehaviour<TutorialManager>
{
    ...
    [SerializeField] private GameObject canSkipIndicator;
    [SerializeField] private TutorialRaycastBlocker overlay;
    [SerializeField] private Camera worldCamera;
    [SerializeField] private List<Canvas> canvasesToDisableAllButtonsIn;
    [SerializeField] private List<Button> individualButtonsToDisable;
    ...
    [SerializeField] public ArrowIndicator arrow; // intentional exposure
    [SerializeField] private float arrowAngleForUI = 0f; // reserved
    [SerializeField] private float arrowAngleForWorld = 0f; // reserved
    [SerializeField] private Vector2 arrowUIOffset = new(0, -40f);
    [SerializeField] private Vector2 arrowWorldOffset = new(0, -2f);

    [SerializeField] private bool overrideDialogBoxTransform;
    [SerializeField] public RectTransformSnapshot defaultDialogBoxTransform;

    [Header("Steps")]
    [SerializeField] private List<SimpleTutorialStep> steps = new();
    public SimpleTutorialStep GetStepAtIndex(int index) => (index >= 0 && index < steps.Count) ? steps[index] : null;
    ...
    [Header("Skip")]
    [SerializeField] private float minDelayBeforeSkip = 0.3f;
    [SerializeField] private List<KeyCode> dialogueSkipKeys = new() { KeyCode.Space, KeyCode.Mouse0 };
    [SerializeField] private bool allowSkipAll = true;
    [SerializeField] private KeyCode skipAllKey = KeyCode.Escape;

    [Header("Clickable Tween")]
    [SerializeField] private bool enableClickableTween = true;
    [SerializeField] private bool enableClickableTweenForButtons = true;
    [SerializeField] private bool enableClickableTweenForWorld = true;
    [SerializeField] private Vector3 targetScale = new(1.2f, 1.2f, 1.2f);
    ...
    private void Update()
    {
        ...
        if (!advanceKeyDown && Mouse.current != null)
            advanceKeyDown = Mouse.current.leftButton.wasPressedThisFrame && dialogueSkipKeys.Contains(KeyCode.Mouse0);
#endif
        if (advanceKeyDown) AdvanceOrSkipDialogue();
    }
    ...
    {
        if (!tutorialRunning || currentStep == null) return;
        switch (currentStep.type)
        {
            case StepType.Dialogue: advanceRequested = true; break;
            case StepType.ClickWorld: worldClickSatisfied = true; break;
            case StepType.ClickButton: buttonClickSatisfied = true; break;
        }
    }
    ...
    {
        if (currentStep != null && currentStep.type == StepType.ClickWorld && clicked && currentStep.worldTarget == clicked)
    ...
    private IEnumerator RunStep(SimpleTutorialStep step)
    {
        ...
        if (useCallbacks) step.preStepEvent?.Invoke();

        ShowDialogue(step);
        RegisterButtonHandler(step);

        if (useCallbacks) step.postStepEvent?.Invoke();

        yield return new WaitForSeconds(minDelayBeforeSkip);
        if (step.type == StepType.Dialogue) canSkipIndicator?.SetActive(true);
        yield return WaitForStepCompletion(step);
        UnregisterButtonHandler(step);
    }

    private IEnumerator WaitForStepCompletion(SimpleTutorialStep step) => step.type switch
    {
    ...
    private void SetupOverlayAndGate(SimpleTutorialStep step)
    {
        if (overlay)
        {
            if (step.type == StepType.Dialogue) overlay.BlockAll(); else overlay.DisableBlocker();
        }
        if (step.type == StepType.ClickWorld) TutorialGate.SetWhitelist(new List<Transform> { step.worldTarget }); else TutorialGate.BlockWorld();
    }

    private void ApplyTweenForStep(SimpleTutorialStep step)
    {
        if (!enableClickableTween) return;
    ...
    private void ApplyArrowForStep(SimpleTutorialStep step)
    {
        ...
        switch (step.type)
        {
            case StepType.ClickWorld:
                if (step.worldTarget) arrow.PointToWorldObject(step.worldTarget, 0f);
                break;
            case StepType.ClickButton:
                if (step.buttonTarget) arrow.PointToUIElement(step.buttonTarget.GetComponent<RectTransform>(), 0f);
                break;
        }
    }
    #endregion
    ...
    private bool ShouldUseTypewriter(SimpleTutorialStep step, string text) =>
        enableTypewriter && !string.IsNullOrEmpty(text) && step != null && step.useTypewriter && GetCharsPerSecond(step) > 0f;
    ...
    {
        if (currentStep == null || currentStep.type != StepType.Dialogue) return;
        if (!dialogueFullyShown) RevealAllDialogue(); else advanceRequested = true;
    }
    ...
    {
        if (step.type != StepType.ClickButton || !step.buttonTarget) return;
        void Handler()
        {
    ...
    {
        if (step.type != StepType.ClickButton || !step.buttonTarget || step.runtimeHandler == null) return;
    ...
    {
        if (tempDisabledButtons.Count == 0) return;
    ...
    private void ApplyClickableTweenTo(Transform target)
    {
        ...
        clickableButtonTween = target.DOScale(newTargetScale, duration)
    ...
    #region Data Types
    [Serializable]
    public enum StepType { Dialogue, ClickWorld, ClickButton }
    ...
    public class SimpleTutorialStep
    {
        ...
        [Header("Arrow")] public bool showArrow = true;
        public bool overrideArrowTransform = true;
        public Vector2 arrowPosition = new(0, 0);
        public float arrowRotation = 0f;
        [Header("Callbacks")] public UnityEvent preStepEvent; public UnityEvent postStepEvent;
        [SerializeField] private bool enableOverrideDialogBoxTransform;
        public RectTransformSnapshot OverrideDialogBoxTransform = null;

        [NonSerialized] public UnityAction runtimeHandler;
        ...
        public static class Prop
        {
            ...
            public const string EnableOverrideDialogBoxTransform = nameof(enableOverrideDialogBoxTransform);
            public const string OverrideDialogBoxTransform = nameof(SimpleTutorialStep.OverrideDialogBoxTransform);
        }
    }
