...
public class TutorialManagerEditor : Editor
{
    ...
    private static readonly GUIContent GC_OverrideDialogBoxTransform = new("Override Dialog Box Transform");
    ...
    private static readonly GUIContent GC_ApplyDialogBoxTransform = new("Apply Default Dialog Box Transform");
    private static readonly GUIContent GC_VisualizeDialogBoxTransform = new("Visualize Default Dialog Box Transform");
    ...
    private void OnEnable()
    {
        ...
        sp_enableClickableTweenForButtons = serializedObject.FindProperty(Prop.EnableClickableTweenForButtons);
        sp_enableClickableTweenForWorld = serializedObject.FindProperty(Prop.EnableClickableTweenForWorld);
    ...
    private void DrawStepsHeader(Rect rect)
    {
        ...
        var importRect = new Rect(exportRect.x - btnGap - importW, rect.y + 1f, importW, rect.height - 2f);
        var collapseRect = new Rect(importRect.x - btnGap - smallBtnW, rect.y + 1f, smallBtnW, rect.height - 2f);
        var expandRect = new Rect(collapseRect.x - btnGap - smallBtnW, rect.y + 1f, smallBtnW, rect.height - 2f);
        ...
        if (GUI.Button(expandRect, "Expand All")) SetAllStepsExpanded(true);
        if (GUI.Button(collapseRect, "Collapse All")) SetAllStepsExpanded(false);
        if (GUI.Button(importRect, "Import JSON")) ImportStepsFromJson();
        if (GUI.Button(exportRect, "Export to JSON")) ExportStepsToJson();
    }

    private float GetStepElementHeight(int index)
    {
        if (!stepsFoldout) return 0f;
        ...
        if (!element.isExpanded) return height + PadBottom;
        ...
        if (sp_overrideDialogBoxPosition.boolValue)
        {
            ...
            var dialogueBoxOverrideProp = element.FindPropertyRelative(StepProp.EnableOverrideDialogBoxTransform);
    ...
    {
        if (!stepsFoldout) return;
        var element = sp_steps.GetArrayElementAtIndex(index);
        float line = EditorGUIUtility.singleLineHeight;
        float vsp = EditorGUIUtility.standardVerticalSpacing;

        var content = new Rect(rect.x + PadLeft, rect.y + PadTop, rect.width - PadLeft, rect.height - (PadTop + PadBottom));
        ...
        EditorGUI.BeginChangeCheck();
        bool newExpanded = EditorGUI.Foldout(foldoutRect, element.isExpanded, $"Step {index + 1} - {typeLabel}", true);
        if (EditorGUI.EndChangeCheck())
        {
        ...
        EditorGUI.PropertyField(new Rect(content.x, y, content.width, dlgH), dialogueProp, GC_Dialogue, true);
        y += dlgH + vsp;
        ...
        if (sp_enableTypewriter.boolValue)
        {
            ...
            if (newUse != useTWProp.boolValue) useTWProp.boolValue = newUse;
            y += line + vsp;
            if (useTWProp.boolValue)
            {
                ...
                var cpsRect = new Rect(row.x + boolWidth + 6f, row.y, row.width - (boolWidth + 6f), row.height);
        ...
        if (typeIndex != StepType_Dialogue && sp_enableArrow.boolValue)
        {
            ...
            if (newShow != showArrowProp.boolValue) showArrowProp.boolValue = newShow;
            y += line + vsp;
            ...
            if (showArrowProp.boolValue)
            {
                ...
                bool newOverride = EditorGUI.ToggleLeft(overrideRect, GC_OverrideArrow, overrideProp.boolValue);
                if (newOverride != overrideProp.boolValue) overrideProp.boolValue = newOverride;
                ...
                if (overrideProp.boolValue)
                {
                    ...
                    if (GUI.Button(btnRect, "Capture Arrow Transform"))
                    {
                        ...
                        {
                            Debug.LogWarning("[TutorialManagerEditor] No ArrowIndicator assigned on TutorialManager.");
                        }
                    }
                    y += line + vsp;
                }
        ...
        if (sp_useCallbacks.boolValue)
        {
            EditorGUI.LabelField(new Rect(content.x, y, content.width, line), GC_Callbacks, EditorStyles.boldLabel);
            y += line + vsp;
        ...
        if (sp_overrideDialogBoxPosition.boolValue)
        {
            var overrideDialogueBoxProp = element.FindPropertyRelative(StepProp.EnableOverrideDialogBoxTransform);
            var overrideDialogueBoxRect = new Rect(content.x, y, content.width, line);
            bool newOverrideDialogueBox = EditorGUI.ToggleLeft(overrideDialogueBoxRect, GC_OverrideDialogBoxTransform, overrideDialogueBoxProp.boolValue);
            ...
            if (overrideDialogueBoxProp.boolValue)
            {
                ...
                Rect applyRect = new Rect(content.x + captureRect.width + 20, y, content.width / 2 - 10, line);
                ...
                if (GUI.Button(captureRect, "Capture Box"))
                {
                    ...
                    if (manager != null)
                    {
                        ...
                        step.OverrideDialogBoxTransform = new RectTransformSnapshot(manager.DialogueBox.transform as RectTransform);
                ...
                if (GUI.Button(applyRect, "Visualize"))
                {
                    ...
                    if (manager.DialogueBox != null)
                    {
                        ...
                        if (step.OverrideDialogBoxTransform.IsEnabled)
                            step.OverrideDialogBoxTransform.ApplyTo(manager.DialogueBox.transform as RectTransform);
                        else
                            manager.defaultDialogBoxTransform?.ApplyTo(manager.DialogueBox.transform as RectTransform);
                    }
                }
                y += line + vsp;
            }
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button(GC_ManagerExport)) ExportManagerToJson();
        if (GUILayout.Button(GC_ManagerImport)) ImportManagerFromJson();
        EditorGUILayout.EndHorizontal();
        EditorGUILayout.Space(6);

        DrawPropertiesExcluding(serializedObject,
            // Typewriter
            Prop.EnableTypewriter, Prop.DefaultTypewriterCps,
            // Arrow
            Prop.EnableArrow, Prop.Arrow, Prop.ArrowAngleForUI, Prop.ArrowAngleForWorld, Prop.ArrowUIOffset, Prop.ArrowWorldOffset,
            // Tween
            Prop.EnableClickableTween, Prop.EnableClickableTweenForButtons, Prop.EnableClickableTweenForWorld, Prop.TargetScale, Prop.Duration, Prop.Loops, Prop.Ease,
            // Steps / callbacks (custom draw below)
            Prop.Steps, Prop.UseCallbacks);
        ...
        EditorGUILayout.Space(6);
        sp_useCallbacks.boolValue = EditorGUILayout.ToggleLeft(GC_UseCallbacks, sp_useCallbacks.boolValue);

        EditorGUILayout.Space(6);
        sp_overrideDialogBoxPosition.boolValue = EditorGUILayout.ToggleLeft(GC_OverrideDialogBoxTransform, sp_overrideDialogBoxPosition.boolValue);
        ...
        if (sp_overrideDialogBoxPosition.boolValue)
        {
            ...
            if (GUILayout.Button(GC_ApplyDialogBoxTransform)) tm.defaultDialogBoxTransform = new RectTransformSnapshot(tm.DialogueBox.transform as RectTransform);
            if (GUILayout.Button(GC_VisualizeDialogBoxTransform)) tm.defaultDialogBoxTransform?.ApplyTo(tm.DialogueBox.transform as RectTransform);
    ...
    private void ExportStepsToJson()
    {
        ...
        for (int i = 0; i < sp_steps.arraySize; i++)
        {
            ...
            bool overrideArrow = element.FindPropertyRelative("overrideArrowTransform")?.boolValue ?? false;
            Vector2 arrowPos = element.FindPropertyRelative("arrowPosition")?.vector2Value ?? Vector2.zero;
            ...
            export.steps.Add(new StepExport
            {
                index = i,
                type = type,
                dialogue = dialogue,
        ...
        WriteJsonWithUniqueName(json, ((TutorialManager)target).gameObject.name + " TutorialSteps.json");
    }
    ...
    private void ImportStepsFromJson()
    {
        try
        {
            ...
            for (int i = 0; i < sp_steps.arraySize; i++)
            {
                ...
                if (useTWProp != null) useTWProp.boolValue = src.useTypewriter;
                dst.FindPropertyRelative("overrideTypewriterSpeed").boolValue = src.overrideTypewriterSpeed;
                dst.FindPropertyRelative("typewriterCharsPerSecond").floatValue = src.typewriterCharsPerSecond;
                ...
                if (typeProp.enumValueIndex == StepType_ClickWorld && !string.IsNullOrEmpty(src.worldTargetPath))
                {
                    var tr = ResolveTransformByPath(src.worldTargetPath);
                    if (tr == null) Debug.LogWarning($"[Tutorial Import] World target not found: '{src.worldTargetPath}'");
                    worldProp.objectReferenceValue = tr;
                }
                else if (typeProp.enumValueIndex == StepType_ClickButton && !string.IsNullOrEmpty(src.uiButtonPath))
                {
                    var tr = ResolveTransformByPath(src.uiButtonPath);
                    Button btn = tr ? tr.GetComponent<Button>() : null;
                    if (btn == null) Debug.LogWarning($"[Tutorial Import] UI Button not found or missing Button component at: '{src.uiButtonPath}'");
            ...
            stepsList.index = (sp_steps.arraySize > 0) ? Mathf.Clamp(stepsList.index, 0, sp_steps.arraySize - 1) : -1;
        ...
        catch (Exception ex)
        {
            ...
            EditorUtility.DisplayDialog("Tutorial Import", "An error occurred. Check the Console for details.", "OK");
        }
    }
    ...
    private void ImportManagerFromJson()
    {
        ...
        catch (Exception ex)
        {
            ...
            EditorUtility.DisplayDialog("Tutorial Manager Import", "An error occurred. Check the Console for details.", "OK");
        }
    }
    ...
    private TutorialManagerFullExport BuildManagerExport()
    {
        ...
        var full = new TutorialManagerFullExport
        {
            ...
            enableClickableTweenForButtons = serializedObject.FindProperty(Prop.EnableClickableTweenForButtons).boolValue,
            enableClickableTweenForWorld = serializedObject.FindProperty(Prop.EnableClickableTweenForWorld).boolValue,
        ...
        for (int i = 0; i < sp_steps.arraySize; i++)
        {
            ...
            full.steps.Add(new StepExport
            {
                ...
                overrideTypewriterSpeed = element.FindPropertyRelative("overrideTypewriterSpeed").boolValue,
                typewriterCharsPerSecond = element.FindPropertyRelative("typewriterCharsPerSecond").floatValue,
    ...
    private void SetBool(string name, bool value) { var p = serializedObject.FindProperty(name); if (p != null) p.boolValue = value; }
    private void SetFloat(string name, float value) { var p = serializedObject.FindProperty(name); if (p != null) p.floatValue = value; }
    private void SetInt(string name, int value) { var p = serializedObject.FindProperty(name); if (p != null) p.intValue = value; }
    private void SetVector2(string name, Vector2 v) { var p = serializedObject.FindProperty(name); if (p != null) p.vector2Value = v; }
    private void SetVector3(string name, Vector3 v) { var p = serializedObject.FindProperty(name); if (p != null) p.vector3Value = v; }
    private void SetEnum(string name, Enum e) { var p = serializedObject.FindProperty(name); if (p != null) p.enumValueIndex = Convert.ToInt32(e); }
    private void SetEase(string name, Ease e) { var p = serializedObject.FindProperty(name); if (p != null) p.enumValueIndex = (int)e; }
    ...
    {
        if (string.IsNullOrEmpty(typeName)) return StepType_Dialogue;
        var key = typeName.Replace(" ", string.Empty).Trim();
        if (string.Equals(key, "Dialogue", StringComparison.OrdinalIgnoreCase)) return StepType_Dialogue;
        if (string.Equals(key, "ClickWorld", StringComparison.OrdinalIgnoreCase)) return StepType_ClickWorld;
        if (string.Equals(key, "ClickButton", StringComparison.OrdinalIgnoreCase)) return StepType_ClickButton;
    ...
    private static Transform ResolveTransformByPath(string path)
    {
        ...
        {
            if (roots[i].name == parts[0]) { rootGo = roots[i]; break; }
        }
    ...
    {
        var tm = (TutorialManager)target;
        if (tm == null || tm.arrow == null) { Debug.LogWarning("[TutorialManagerEditor] Cannot visualize (no TutorialManager.arrow)"); return; }

        // Hide first to emulate runtime behaviour
        tm.arrow.Hide();

        var showArrowProp = stepElement.FindPropertyRelative(StepProp.ShowArrow);
        if (!showArrowProp.boolValue) return; // nothing to show
        ...
        if (stepTypeIndex == StepType_ClickWorld)
        {
            ...
            if (tr) tm.arrow.PointToWorldObject(tr, 0f);
        }
        else if (stepTypeIndex == StepType_ClickButton)
        {
            ...
            if (btn && btn.TryGetComponent(out RectTransform rt)) tm.arrow.PointToUIElement(rt, 0f);
        }
    }
