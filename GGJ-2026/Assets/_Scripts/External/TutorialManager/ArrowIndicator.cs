...
public class ArrowIndicator : MonoBehaviour
{
    ...
    [SerializeField] private ArrowPlacement direction = ArrowPlacement.Bottom;
    ...
    public void PointToWorldObject(Transform target, float? angleDeg)
    {
        ...
        if (target.TryGetComponent(out Renderer rend))
        {
            ...
            switch (direction)
            {
                case ArrowPlacement.Top: worldPos.y += ext.y; break;
                case ArrowPlacement.Bottom: worldPos.y -= ext.y; break;
                case ArrowPlacement.Left: worldPos.x -= ext.x; break;
                case ArrowPlacement.Right: worldPos.x += ext.x; break;
            }
        }
    ...
    public void PointToUIElement(RectTransform target, float? angleDeg)
    {
        ...
        if (ScreenToLocal(screenPos, out Vector2 localPoint))
        {
            ...
            switch (direction)
            {
                case ArrowPlacement.Top: localPoint.y += target.rect.height / 2f; break;
                case ArrowPlacement.Bottom: localPoint.y -= target.rect.height / 2f; break;
                case ArrowPlacement.Left: localPoint.x -= target.rect.width / 2f; break;
                case ArrowPlacement.Right: localPoint.x += target.rect.width / 2f; break;
            }
            localPoint += ArrowSizeOffset();
            ApplyAnchoredPosition(localPoint, angleDeg);
        }
    ...
    public void SetRotation(float angleDeg)
    {
        ...
        var e = arrowRect.localEulerAngles; e.z = angleDeg; arrowRect.localEulerAngles = e;
    }
    ...
    {
        EnsureRect();
        if (angleDeg.HasValue) SetRotation(angleDeg.Value);
        if (arrowRect == null) return;
    ...
    private bool ScreenToLocal(Vector2 screenPos, out Vector2 localPoint)
    {
        EnsureRect();
        localPoint = default;
        if (canvas == null || arrowRect == null) return false;
        var root = canvas.transform as RectTransform;
        Camera cam = canvas.renderMode == RenderMode.ScreenSpaceOverlay ? null : GetUICamera();
        return RectTransformUtility.ScreenPointToLocalPointInRectangle(root, screenPos, cam, out localPoint);
    }

    /// <summary>
    /// Returns the camera used for UI calculations (canvas worldCamera or main).
    /// </summary>
    private Camera GetUICamera() => canvas != null && canvas.worldCamera != null ? canvas.worldCamera : Camera.main;
    ...
    {
        if (graphic != null) graphic.gameObject.SetActive(on);
        else gameObject.SetActive(on);
    }

    public enum ArrowPlacement { Top, Bottom, Left, Right }
}
